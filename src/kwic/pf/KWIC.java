 ...
 */
  ...
  public static void main(String[] args){
    ...
    KWIC kwic = new KWIC();
//    kwic.execute(args[0]);
    kwic.executeRemoveNoise(args[0], args[1]);
  }

  public void executeRemoveNoise(String file, String noiseFile){
    try{
      Pipe in_cs = new Pipe();
      Pipe cs_sf= new Pipe();
      Pipe sf_cp1 = new Pipe();
      Pipe cp1_al = new Pipe();
      Pipe al_cp2 = new Pipe();
      Pipe cp2_ou = new Pipe();

      FileInputStream in = new FileInputStream(file);

      Input input = new Input(in, in_cs);
      CircularShifter shifter = new CircularShifter(in_cs, cs_sf);
      ShiftFilter shiftFilter = new ShiftFilter(cs_sf, sf_cp1, noiseFile);
      Capitalizer capitalizer1 = new Capitalizer(sf_cp1, cp1_al);
      Alphabetizer alpha = new Alphabetizer(cp1_al, al_cp2);
      Capitalizer capitalizer2 = new Capitalizer(al_cp2, cp2_ou);
      Output output = new Output(cp2_ou);
